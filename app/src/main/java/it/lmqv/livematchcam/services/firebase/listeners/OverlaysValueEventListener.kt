    ...
    private fun handleValueEventListener() {
        ...
        if (this.accountKey.isNotEmpty() && this.key.isNotEmpty()) {
            ...
            this.overlaysRef!!.get().addOnSuccessListener { snapshot ->
                if (snapshot.exists()) {
                    val overlaysModel =
                        snapshot.getValue(OverlaysModel::class.java) ?: OverlaysModel()
                    ...
                    val defaultOverlays = OverlaysModel()
                    overlaysRef!!.setValue(defaultOverlays).addOnSuccessListener {
                        onChangeListener?.onChangeScoreboard(defaultOverlays.scoreboard)
                        onChangeListener?.onChangeFilters(emptyList())
                    }
                }
